...
static int NUMANIMS[NUMEPISODES] = {arrlen(epsd0animinfo), arrlen(epsd1animinfo), arrlen(epsd2animinfo),};
...
void WI_drawOnLnode(const int n, patch_t* c[])
{
    boolean fits = false;
    ...
    if (fits && i < 2) { V_DrawPatch(lnodes[wbs->epsd][n].x, lnodes[wbs->epsd][n].y, c[i]); }
    else
    {
        // DEBUG
...
void WI_initAnimatedBack(void)
{
    ...
    for (int i = 0; i < NUMANIMS[wbs->epsd]; i++)
    {
        ...
        if (a->type == ANIM_ALWAYS) a->nexttic = bcnt + 1 + M_Random() % a->period;
        else if (a->type == ANIM_RANDOM) a->nexttic = bcnt + 1 + a->data2 + M_Random() % a->data1;
...
int WI_drawNum(int x, const int y, int n, int digits)
{
...
void WI_drawPercent(const int x, const int y, const int p)
{
    if (p < 0) return;
...
//
void WI_drawTime(int x, const int y, const int t)
{
    if (t < 0) return;
...
void WI_drawShowNextLoc(void)
{
    ...
    if (gamemode != commercial)
    {
        ...
        for (int i = 0; i <= last; i++) WI_drawOnLnode(i, splat);
...
int WI_fragSum(const int playernum)
{
    int frags = 0;
...
void WI_initDeathmatchStats(void)
{
    ...
    for (int i = 0; i < MAXPLAYERS; i++)
    {
        if (playeringame[i])
        {
            for (int j = 0; j < MAXPLAYERS; j++) if (playeringame[j]) dm_frags[i][j] = 0;
...
void WI_updateDeathmatchStats(void)
{
    ...
    if (acceleratestage && dm_state != 4)
    {
        ...
        for (i = 0; i < MAXPLAYERS; i++)
        {
            if (playeringame[i])
            {
                for (j = 0; j < MAXPLAYERS; j++) if (playeringame[j]) dm_frags[i][j] = plrs[i].frags[j];
    ...
    if (ng_state == 2)
    {
        ...
        for (i = 0; i < MAXPLAYERS; i++)
        {
            ...
            if (cnt_kills[i] >= plrs[i].skills * 100 / wbs->maxkills)
                cnt_kills[i] = plrs[i].skills * 100 / wbs->
                    maxkills;
    ...
    else if (ng_state == 4)
    {
        ...
        for (i = 0; i < MAXPLAYERS; i++)
        {
            ...
            if (cnt_items[i] >= plrs[i].sitems * 100 / wbs->maxitems)
                cnt_items[i] = plrs[i].sitems * 100 / wbs->
                    maxitems;
            else stillticking = true;
        }
        if (!stillticking)
    ...
    else if (ng_state == 6)
    {
        ...
        for (i = 0; i < MAXPLAYERS; i++)
        {
            ...
            if (cnt_secret[i] >= plrs[i].ssecret * 100 / wbs->maxsecret)
                cnt_secret[i] = plrs[i].ssecret * 100 / wbs
                    ->maxsecret;
            else stillticking = true;
        }
...
static void WI_loadUnloadData(const load_callback_t callback)
{
    int i;
    char name[9];
    ...
    {
        // "items"
        if (netgame && !deathmatch) callback(DEH_String("WIOBJ"), &items);
        else callback(DEH_String("WIOSTI"), &items);
    }
    else { callback(DEH_String("WIOSTI"), &items); }
    ...
    if (gamemode == commercial) { M_StringCopy(name, DEH_String("INTERPIC"), sizeof(name)); }
    else if (gamemode == retail && wbs->epsd == 3) { M_StringCopy(name, DEH_String("INTERPIC"), sizeof(name)); }
...
static void WI_loadCallback(char* name, patch_t** variable) { *variable = W_CacheLumpName(name, PU_STATIC); }
...
void WI_loadData(void)
{
    ...
    else { lnames = (patch_t**)Z_Malloc(sizeof(patch_t*) * NUMMAPS, PU_STATIC, NULL); }
...
void WI_initVariables(wbstartstruct_t* wbstartstruct)
{
    ...
    } RNGCHECK(wbs->pnum, 0, MAXPLAYERS); RNGCHECK(wbs->pnum, 0, MAXPLAYERS);
#endif
    ...
    if (gamemode != retail) if (wbs->epsd > 2) wbs->epsd -= 3;
}
