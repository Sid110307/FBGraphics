...
int P_DivlineSide(const fixed_t x, const fixed_t y, const divline_t* node)
{
    ...
    const fixed_t left = (node->dy >> FRACBITS) * (dx >> FRACBITS);
    const fixed_t right = (dy >> FRACBITS) * (node->dx >> FRACBITS);
...
boolean P_CrossSubsector(const int num)
{
    ...
    for (; count; seg++, count--)
    {
        ...
        // because of ceiling height differences
        if (front->floorheight > back->floorheight) openbottom = front->floorheight;
