...
//
int cht_CheckCheat(cheatseq_t* cht, const char key)
{
    ...
    if (cht->chars_read < strlen(cht->sequence))
    {
        ...
        if (key == cht->sequence[cht->chars_read]) ++cht->chars_read;
...
void cht_GetParam(const cheatseq_t* cht, char* buffer) { memcpy(buffer, cht->parameter_buf, cht->parameter_chars); }
