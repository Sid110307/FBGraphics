...
char* P_TempSaveGameFile(void)
{
    ...
    if (filename == NULL) { filename = M_StringJoin(savegamedir, "temp.dsg", NULL); }
...
char* P_SaveGameFile(const int slot)
{
    static char* filename = NULL;
...
static void saveg_write8(const byte value)
{
...
static void saveg_write16(const short value)
{
...
static void saveg_write32(const int value)
{
    saveg_write8(value & 0xff);
    saveg_write8(value >> 8 & 0xff);
...
{
    const unsigned long pos = ftell(save_stream);

    const int padding = 4 - (pos & 3) & 3;

    for (int i = 0; i < padding; ++i) { saveg_read8(); }
}
...
static void saveg_write_pad(void)
{
    ...
    for (int i = 0; i < padding; ++i) { saveg_write8(0); }
}
...
static void saveg_write_actionf_t(const actionf_t* str)
{
    // actionf_p1 acp1;
...
static void saveg_write_mobj_t(mobj_t* str)
{
    ...
    // struct player_s* player;
    if (str->player) { saveg_write32(str->player - players + 1); }
    else { saveg_write32(0); }
...
static void saveg_read_pspdef_t(pspdef_t* str)
{
    ...
    if (state > 0) { str->state = &states[state]; }
    else { str->state = NULL; }
...
{
    // state_t* state;
    if (str->state) { saveg_write32(str->state - states); }
    else { saveg_write32(0); }
...
static void saveg_read_player_t(player_t* str)
{
    ...
    for (i = 0; i < NUMPOWERS; ++i) { str->powers[i] = saveg_read32(); }

    // boolean cards[NUMCARDS];
    for (i = 0; i < NUMCARDS; ++i) { str->cards[i] = saveg_read32(); }

    // boolean backpack;
    str->backpack = saveg_read32();

    // int frags[MAXPLAYERS];
    for (i = 0; i < MAXPLAYERS; ++i) { str->frags[i] = saveg_read32(); }
    ...
    for (i = 0; i < NUMWEAPONS; ++i) { str->weaponowned[i] = saveg_read32(); }

    // int ammo[NUMAMMO];
    for (i = 0; i < NUMAMMO; ++i) { str->ammo[i] = saveg_read32(); }

    // int maxammo[NUMAMMO];
    for (i = 0; i < NUMAMMO; ++i) { str->maxammo[i] = saveg_read32(); }
    ...
    for (i = 0; i < NUMPSPRITES; ++i) { saveg_read_pspdef_t(&str->psprites[i]); }
...
static void saveg_write_player_t(player_t* str)
{
    ...
    // int powers[NUMPOWERS];
    for (i = 0; i < NUMPOWERS; ++i) { saveg_write32(str->powers[i]); }

    // boolean cards[NUMCARDS];
    for (i = 0; i < NUMCARDS; ++i) { saveg_write32(str->cards[i]); }

    // boolean backpack;
    saveg_write32(str->backpack);

    // int frags[MAXPLAYERS];
    for (i = 0; i < MAXPLAYERS; ++i) { saveg_write32(str->frags[i]); }
    ...
    // boolean weaponowned[NUMWEAPONS];
    for (i = 0; i < NUMWEAPONS; ++i) { saveg_write32(str->weaponowned[i]); }

    // int ammo[NUMAMMO];
    for (i = 0; i < NUMAMMO; ++i) { saveg_write32(str->ammo[i]); }

    // int maxammo[NUMAMMO];
    for (i = 0; i < NUMAMMO; ++i) { saveg_write32(str->maxammo[i]); }
    ...
    // pspdef_t psprites[NUMPSPRITES];
    for (i = 0; i < NUMPSPRITES; ++i) { saveg_write_pspdef_t(&str->psprites[i]); }
...
boolean P_ReadSaveGameHeader(void)
{
    ...
    // get the times
    const byte a = saveg_read8();
    const byte b = saveg_read8();
    const byte c = saveg_read8();
...
void P_ArchiveSpecials(void)
{
    ...
    for (thinker_t* th = thinkercap.next; th != &thinkercap; th = th->next)
    {
        ...
        {
            for (i = 0; i < MAXCEILINGS; i++) if (activeceilings[i] == (ceiling_t*)th) break;
