...
//
void P_Thrust(const player_t* player, angle_t angle, const fixed_t move)
{
...
void P_CalcHeight(player_t* player)
{
    ...
    const int angle = FINEANGLES / 20 * leveltime & FINEMASK;
    const fixed_t bob = FixedMul(player->bob / 2, finesine[angle]);
...
void P_PlayerThink(player_t* player)
{
    ...
    if (cmd->buttons & BT_CHANGE)
    {
        ...
        if (newweapon == wp_fist && player->weaponowned[wp_chainsaw] && !(player->readyweapon == wp_chainsaw && player->
            powers[pw_strength])) { newweapon = wp_chainsaw; }

        if (gamemode == commercial && newweapon == wp_shotgun && player->weaponowned[wp_supershotgun] && player->
            readyweapon != wp_supershotgun) { newweapon = wp_supershotgun; }
    ...
    if (player->powers[pw_invisibility]) if (!--player->powers[pw_invisibility]) player->mo->flags &= ~MF_SHADOW;
