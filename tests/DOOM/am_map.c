...
mline_t cheat_player_arrow[] = {
    ...
    {{-R / 2, -R / 6}, {-R / 2 + R / 6, -R / 6}}, {{-R / 2 + R / 6, -R / 6}, {-R / 2 + R / 6, R / 4}},
    {{-R / 6, 0}, {-R / 6, -R / 6}}, // >>-dd-->
    {{-R / 6, -R / 6}, {0, -R / 6}}, {{0, -R / 6}, {0, R / 4}}, {{R / 6, R / 4}, {R / 6, -R / 7}}, // >>-ddt->
    {{R / 6, -R / 7}, {R / 6 + R / 32, -R / 7 - R / 32}}, {{R / 6 + R / 32, -R / 7 - R / 32}, {R / 6 + R / 10, -R / 7}}
};
#undef R
...
mline_t triangle_guy[] = {
    ...
    {{(fixed_t)(.867 * R), (fixed_t)(-.5 * R)}, {(fixed_t)0, (R)}},
    {{(fixed_t)0, (R)}, {(fixed_t)(-.867 * R), (fixed_t)(-.5 * R)}}
};
#undef R

#define R (FRACUNIT)
mline_t thintriangle_guy[] = {
    {{(fixed_t)(-.5 * R), (fixed_t)(-.7 * R)}, {(R), (fixed_t)0}},
    {{(R), (fixed_t)0}, {(fixed_t)(-.5 * R), (fixed_t)(.7 * R)}},
...
void AM_getIslope(const mline_t* ml, islope_t* is)
{
    const int dy = ml->a.y - ml->b.y;
    const int dx = ml->b.x - ml->a.x;
...
{
    for (int i = 0; i < AM_NUMMARKPOINTS; i++) markpoints[i].x = -1; // means empty
    markpointnum = 0;
}
...
boolean AM_Responder(const event_t* ev)
{
    ...
    else if (ev->type == ev_keydown)
    {
        ...
        else if (key == key_map_follow)
        {
            ...
            f_oldloc.x = INT_MAX;
            if (followplayer) plr->message = DEH_String(AMSTR_FOLLOWON);
            else plr->message = DEH_String(AMSTR_FOLLOWOFF);
        }
        else if (key == key_map_grid)
        {
            grid = !grid;
            if (grid) plr->message = DEH_String(AMSTR_GRIDON);
            else plr->message = DEH_String(AMSTR_GRIDOFF);
        }
    ...
    else if (ev->type == ev_keyup)
    {
        ...
        if (key == key_map_east) { if (!followplayer) m_paninc.x = 0; }
        else if (key == key_map_west) { if (!followplayer) m_paninc.x = 0; }
        else if (key == key_map_north) { if (!followplayer) m_paninc.y = 0; }
        else if (key == key_map_south) { if (!followplayer) m_paninc.y = 0; }
...
//
void AM_clearFB(const int color) { memset(fb, color, f_w * f_h); }
...
//
boolean AM_clipMline(const mline_t* ml, fline_t* fl)
{
    enum
    {
        LEFT   = 1,
...
//
void AM_drawFline(const fline_t* fl, const int color)
{
    register int d;
    ...
    // For debugging only
    if (fl->a.x < 0 || fl->a.x >= f_w || fl->a.y < 0 || fl->a.y >= f_h || fl->b.x < 0 || fl->b.x >= f_w || fl->b.y < 0
        || fl->b.y >= f_h)
    {
...
#define PUTDOT(xx, yy, cc) fb[(yy)*f_w+(xx)]=(cc)
    ...
    const register int dx = fl->b.x - fl->a.x;
    const register int ax = 2 * (dx < 0 ? -dx : dx);
    const register int sx = dx < 0 ? -1 : 1;

    const register int dy = fl->b.y - fl->a.y;
    const register int ay = 2 * (dy < 0 ? -dy : dy);
    const register int sy = dy < 0 ? -1 : 1;

    register int x = fl->a.x;
    register int y = fl->a.y;
...
//
void AM_drawMline(mline_t* ml, const int color)
{
    static fline_t fl;
...
void AM_drawGrid(const int color)
{
    ...
    if ((start - bmaporgx) % (MAPBLOCKUNITS << FRACBITS))
        start += (MAPBLOCKUNITS << FRACBITS) - (start - bmaporgx) % (
    ...
    if ((start - bmaporgy) % (MAPBLOCKUNITS << FRACBITS))
        start += (MAPBLOCKUNITS << FRACBITS) - (start - bmaporgy) % (
            MAPBLOCKUNITS << FRACBITS);
    end = m_y + m_h;
...
void AM_drawWalls(void)
{
    ...
    for (int i = 0; i < numlines; i++)
    {
        ...
        else if (plr->powers[pw_allmap]) { if (!(lines[i].flags & LINE_NEVERSEE)) AM_drawMline(&l, GRAYS + 3); }
    }
}
...
void AM_drawLineCharacter(const mline_t* lineguy, const int lineguylines, const fixed_t scale, const angle_t angle,
                          const int color, const fixed_t x, const fixed_t y)
{
    mline_t l;
...
void AM_drawPlayers(void)
{
    ...
    {
        if (cheating)
            AM_drawLineCharacter(cheat_player_arrow, arrlen(cheat_player_arrow), 0, plr->mo->angle, WHITE, plr->mo->x,
                                 plr->mo->y);
        else AM_drawLineCharacter(player_arrow, arrlen(player_arrow), 0, plr->mo->angle, WHITE, plr->mo->x, plr->mo->y);
        return;
    }
...
void AM_drawMarks(void)
{
    for (int i = 0; i < AM_NUMMARKPOINTS; i++)
    {
        if (markpoints[i].x != -1)
        {
            ...
            const int fx = CXMTOF(markpoints[i].x);
            const int fy = CYMTOF(markpoints[i].y);
            if (fx >= f_x && fx <= f_w - w && fy >= f_y && fy <= f_h - h) V_DrawPatch(fx, fy, marknums[i]);
        }
    }
}
