...
{
    const unsigned int result = gus_ram_kb / 256;
...
static int SplitLine(char* line, char** fields, const unsigned int max_fields)
{
    char* p;
...
static void ParseLine(gus_config_t* config, char* line)
{
    ...
    const unsigned int num_fields = SplitLine(line, fields, 6);

    if (num_fields < 6) { return; }

    const unsigned int instr_id = atoi(fields[0]);
    const unsigned int mapped_id = atoi(fields[MappingIndex()]);
...
static void ParseDMXConfig(char* dmxconf, gus_config_t* config)
{
    ...
    for (unsigned int i = 0; i < MAX_INSTRUMENTS; ++i) { config->mapping[i] = -1; }
...
static void FreeDMXConfig(const gus_config_t* config)
{
    for (unsigned int i = 0; i < MAX_INSTRUMENTS; ++i) { free(config->patch_names[i]); }
}
...
static char* ReadDMXConfig(void)
{
    ...
    if (lumpnum < 0) { lumpnum = W_GetNumForName("DMXGUSC"); }

    const unsigned int len = W_LumpLength(lumpnum);
    char* data = Z_Malloc(len + 1, PU_STATIC, NULL);
...
static boolean WriteTimidityConfig(const char* path, const gus_config_t* config)
{
    ...
    for (i = 0; i < 128; ++i)
    {
        if (config->mapping[i] >= 0 && config->mapping[i] < MAX_INSTRUMENTS && config->patch_names[config->mapping[i]]
            != NULL) { fprintf(fstream, "%i %s\n", i, config->patch_names[config->mapping[i]]); }
    }
...
boolean GUS_WriteConfig(char* path)
{
    ...
    const boolean result = WriteTimidityConfig(path, &config);
