...
{
    for (int i = 0; i < num_open_wadfiles; ++i) { if (open_wadfiles[i] == handle) { return i; } }
    ...
    const int result = num_open_wadfiles;
    ++num_open_wadfiles;

    return result;
}

static void ChecksumAddLump(sha1_context_t* sha1_context, const lumpinfo_t* lump)
{
    char buf[9];
...
void W_Checksum(sha1_digest_t digest)
{
    ...
    for (unsigned int i = 0; i < numlumps; ++i) { ChecksumAddLump(&sha1_context, &lumpinfo[i]); }
