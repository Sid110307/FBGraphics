...
static boolean BuildNewTic(void)
{
    ...
    const int gameticdiv = gametic / ticdup;
    ...
    if (net_client_connected) { NET_CL_SendTiccmd(&cmd, maketic); }
...
void NetUpdate(void)
{
    ...
    for (int i = 0; i < newtics; i++) { if (!BuildNewTic()) { break; } }
}
...
static void D_Disconnected(void)
{
    ...
    if (drone) { I_Error("Disconnected from server in drone mode."); }
...
static void BlockUntilStart(net_gamesettings_t* settings, netgame_startup_callback_t callback)
{
    while (!NET_CL_GetSettings(settings))
    {
        ...
        if (!net_client_connected) { I_Error("Lost connection to server"); }
...
{
#if ORIGCODE
    int i; offsetms = 0; recvtic = 0; settings->consoleplayer = 0; settings->num_players = 1; settings->player_classes[
        0] = player_class;
    ...
    // has some bugs.
    //
    if (M_CheckParm("-newsync") > 0) settings->new_sync = 1;
    else settings->new_sync = 0;
    ...
    i = M_CheckParmWithArgs("-extratics", 1); if (i > 0) settings->extratics = atoi(myargv[i + 1]);
    else settings->extratics = 1;
    ...
    i = M_CheckParmWithArgs("-dup", 1); if (i > 0) settings->ticdup = atoi(myargv[i + 1]);
    else settings->ticdup = 1; if (net_client_connected)
    {
    ...
    localplayer = settings->consoleplayer; for (i = 0; i < NET_MAXPLAYERS; ++i)
    {
    ...
    ticdup = settings->ticdup; new_sync = settings->new_sync;
...
boolean D_InitNetGame(net_connect_data_t* connect_data)
{
    ...
    else
    {
        ...
        if (i > 0)
        {
            ...
            if (addr == NULL) { I_Error("No server found on local LAN"); }
        }
        ...
        if (i > 0)
        {
            ...
            if (addr == NULL) { I_Error("Unable to resolve '%s'\n", myargv[i + 1]); }
        }
    } if (addr != NULL)
    {
        if (M_CheckParm("-drone") > 0) { connect_data->drone = true; }
...
{
#ifdef FEATURE_MULTIPLAYER
    NET_SV_Shutdown(); NET_CL_Disconnect();
#endif
}
...
#ifdef FEATURE_MULTIPLAYER
    if (net_client_connected) { if (drone || recvtic < lowtic) { lowtic = recvtic; } }
#endif
...
static boolean PlayersInGame(void)
{
    ...
    {
        for (unsigned int i = 0; i < NET_MAXPLAYERS; ++i) { result = result || local_playeringame[i]; }
    }

    // Whether single or multi-player, unless we are running as a drone,
    // we are in the game.

    if (!drone) { result = true; }
...
{
    for (unsigned int i = 0; i < NET_MAXPLAYERS; ++i) { if (i != localplayer) { set->ingame[i] = false; } }
}
...
void TryRunTics(void)
{
    ...
    // get real tics
    const int entertic = I_GetTime() / ticdup;
    const int realtics = entertic - oldentertics;
    ...
    const int availabletics = lowtic - gametic / ticdup;

    // decide how many tics to run

    if (new_sync) { counts = availabletics; }
    else
    {
        // decide how many tics to run
        ...
        if (net_client_connected) { OldNetSync(); }
    }
    ...
    while (!PlayersInGame() || lowtic < gametic / ticdup + counts)
    {
        ...
        if (I_GetTime() / ticdup - entertic > 0) { return; }
    ...
    while (counts--)
    {
        ...
        if (!net_client_connected) { SinglePlayerClear(set); }
