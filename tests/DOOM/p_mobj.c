...
void P_XYMovement(mobj_t* mo)
{
    ...
    do
    {
        ...
        if (!P_TryMove(mo, ptryx, ptryy))
        {
            ...
            {
                // explode a missile
                if (ceilingline&& ceilingline


                ->
                backsector&& ceilingline
                ->
    ...
    if (mo->momx > -STOPSPEED && mo->momx < STOPSPEED && mo->momy > -STOPSPEED && mo->momy < STOPSPEED && (!player || (
...
void P_ZMovement(mobj_t* mo)
{
    ...
    if (mo->z <= mo->floorz)
    {
        ...
        const int correct_lost_soul_bounce = gameversion >= exe_ultimate;
...
void P_NightmareRespawn(mobj_t* mobj)
{
    ...
    const fixed_t x = mobj->spawnpoint.x << FRACBITS;
    const fixed_t y = mobj->spawnpoint.y << FRACBITS;
    ...
    mobj_t* mo = P_SpawnMobj(mobj->x, mobj->y, mobj->subsector->sector->floorheight, MT_TFOG);
...
void P_MobjThinker(mobj_t* mobj)
{
    ...
    if (mobj->tics != -1)
    {
        ...
        if (!mobj->tics) if (!P_SetMobjState(mobj, mobj->state->nextstate)) return; // freed itself
    }
    else
    {
...
void P_RespawnSpecials(void)
{
    ...
    for (i = 0; i < NUMMOBJTYPES; i++) { if (mthing->type == mobjinfo[i].doomednum) break; }
...
void P_SpawnPlayer(const mapthing_t* mthing)
{
    ...
    if (deathmatch) for (int i = 0; i < NUMCARDS; i++) p->cards[i] = true;
...
void P_SpawnMapThing(mapthing_t* mthing)
{
    ...
    if (i == NUMMOBJTYPES) I_Error("P_SpawnMapThing: Unknown type %i at (%i, %i)", mthing->type, mthing->x, mthing->y);

    // don't spawn keycards and players in deathmatch
    if (deathmatch&& mobjinfo[i]
    ...
    if (nomonsters && (i == MT_SKULL || mobjinfo[i].flags & MF_COUNTKILL)) { return; }
...
void P_SpawnPuff(const fixed_t x, const fixed_t y, fixed_t z)
{
...
// P_SpawnBlood
//
void P_SpawnBlood(const fixed_t x, const fixed_t y, fixed_t z, const int damage)
{
    z += (P_Random() - P_Random()) << 10;
...
// P_SpawnMissile
//
mobj_t* P_SpawnMissile(mobj_t* source, const mobj_t* dest, const mobjtype_t type)
{
...
void P_SpawnPlayerMissile(mobj_t* source, const mobjtype_t type)
{
    ...
    const fixed_t x = source->x;
    const fixed_t y = source->y;
    const fixed_t z = source->z + 4 * 8 * FRACUNIT;
