...
// TELEPORTATION
//
int EV_Teleport(const line_t* line, const int side, mobj_t* thing)
{
    ...
    for (int i = 0; i < numsectors; i++)
    {
        if (sectors[i].tag == tag)
        {
            ...
            for (thinker = thinkercap.next; thinker != &thinkercap; thinker = thinker->next)
            {
                ...
                const mobj_t* m = (mobj_t*)thinker;

                // not a teleportman
                if (m->type != MT_TELEPORTMAN) continue;

                const sector_t* sector = m->subsector->sector;
                // wrong sector
