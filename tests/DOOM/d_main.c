...
void D_DoomLoop(void)
{
    ...
    if (testcontrols) { wipegamestate = gamestate; }
...
//
void D_PageTicker(void) { if (--pagetic < 0) D_AdvanceDemo(); }
...
void D_DoAdvanceDemo(void)
{
    ...
    if (gameversion == exe_ultimate || gameversion == exe_final) demosequence = (demosequence + 1) % 7;
    ...
    switch (demosequence)
    {
        ...
        case 4: gamestate = GS_DEMOSCREEN;
            ...
            else
            {
                ...
                if (gamemode == retail) pagename = DEH_String("CREDIT");
...
static char* GetGameName(char* gamename)
{
    for (size_t i = 0; i < arrlen(banners); ++i)
    {
        ...
        const char* deh_sub = DEH_String(banners[i]);
        ...
        if (deh_sub != banners[i])
        {
            ...
            const size_t gamename_size = strlen(deh_sub) + 10;
            gamename = Z_Malloc(gamename_size, PU_STATIC, 0);
            const int version = G_VanillaVersionCode();
            M_snprintf(gamename, gamename_size, deh_sub, version / 100, version % 100);

            while (gamename[0] != '\0' && isspace(gamename[0])) { memmove(gamename, gamename + 1, gamename_size - 1); }
...
static void SetMissionForPackName(char* pack_name)
{
    ...
    static const struct
    {
        char* name;
        int mission;
    } packs[] = {{"doom2", doom2}, {"tnt", pack_tnt}, {"plutonia", pack_plut},};
...
void D_IdentifyVersion(void)
{
    ...
    if (logical_gamemission == doom)
    {
        ...
        else if (W_CheckNumForName("E3M1") > 0) { gamemode = registered; }
    ...
    else
    {
        ...
        // "tnt" and "plutonia".
        //
        const int p = M_CheckParmWithArgs("-pack", 1);
        if (p > 0) { SetMissionForPackName(myargv[p + 1]); }
    }
}
...
{
    const boolean is_freedoom = W_CheckNumForName("FREEDOOM") >= 0, is_freedm = W_CheckNumForName("FREEDM") >= 0;
    ...
    if (logical_gamemission == doom)
    {
        ...
        if (is_freedoom) { gamedescription = GetGameName("Freedoom: Phase 1"); }
        ...
        else if (gamemode == registered) { gamedescription = GetGameName("DOOM Registered"); }
        else if (gamemode == shareware) { gamedescription = GetGameName("DOOM Shareware"); }
    }
    else
    {
        ...
        if (is_freedoom)
        {
            if (is_freedm) { gamedescription = GetGameName("FreeDM"); }
            else { gamedescription = GetGameName("Freedoom: Phase 2"); }
        }
        else if (logical_gamemission == doom2) { gamedescription = GetGameName("DOOM 2: Hell on Earth"); }
        else if (logical_gamemission == pack_plut) { gamedescription = GetGameName("DOOM 2: Plutonia Experiment"); }
        else if (logical_gamemission == pack_tnt) { gamedescription = GetGameName("DOOM 2: TNT - Evilution"); }
    }
}
...
{
    printf(" adding %s\n", filename);
    const wad_file_t* handle = W_AddFile(filename);
...
void PrintDehackedBanners(void)
{
    ...
    {
        char* deh_s = DEH_String(copyright_banners[i]);
...
static struct
{
    ...
    {"Doom 1.666", "1.666", exe_doom_1_666}, {"Doom 1.7/1.7a", "1.7", exe_doom_1_7}, {"Doom 1.8", "1.8", exe_doom_1_8},
    {"Doom 1.9", "1.9", exe_doom_1_9}, {"Hacx", "hacx", exe_hacx}, {"Ultimate Doom", "ultimate", exe_ultimate},
    {"Final Doom", "final", exe_final}, {"Final Doom (alt)", "final2", exe_final2}, {"Chex Quest", "chex", exe_chex},
    {NULL, NULL, 0},
};
...
static void InitGameVersion(void)
{
    ...
    const int p = M_CheckParmWithArgs("-gameversion", 1);
    ...
    else
    {
        ...
        else if (gamemode == retail) { gameversion = exe_ultimate; }
        else if (gamemode == commercial)
        {
            if (gamemission == doom2) { gameversion = exe_doom_1_9; }
            else
            {
    ...
    if (gameversion < exe_ultimate && gamemode == retail) { gamemode = registered; }
...
static void D_Endoom(void)
{
    ...
    if (!show_endoom || !main_loop_started || screensaver_mode || M_CheckParm("-testcontrols") > 0) { return; }

    byte* endoom = W_CacheLumpName(DEH_String("ENDOOM"), PU_STATIC);
...
static void LoadIwadDeh(void)
{
    ...
    if (gameversion == exe_chex)
    {
        ...
        if (!DEH_LoadFile(chex_deh)) { I_Error("Failed to load chex.deh needed for emulating chex.exe."); }
    }
}
#endif
...
void D_DoomMain(void)
{
    ...
    p = M_CheckParmWithArgs("-query", 1); if (p)
    {
    ...
    if (p)
    {
        ...
        if (M_StringEndsWith(myargv[p + 1], ".lmp")) { M_StringCopy(file, myargv[p + 1], sizeof(file)); }
        else { DEH_snprintf(file, sizeof(file), "%s.lmp", myargv[p + 1]); }

        if (D_AddFile(file)) { M_StringCopy(demolumpname, lumpinfo[numlumps - 1].name, sizeof(demolumpname)); }
        else
        {
    ...
    if (M_ParmExists("-cdrom")) { savegamedir = configdir; }
    else
#endif
    {
    ...
    if (modifiedgame)
    {
        ...
        const char name[23][8] = {
            "e2m1", "e2m2", "e2m3", "e2m4", "e2m5", "e2m6", "e2m7", "e2m8", "e2m9", "e3m1", "e3m3", "e3m3", "e3m4",
            "e3m5", "e3m6", "e3m7", "e3m8", "e3m9", "dphoof", "bfgga0", "heada1", "cybra1", "spida1d1"
        };

        if (gamemode == shareware) I_Error(DEH_String("\nYou cannot -file with the shareware " "version. Register!"));
    ...
    if (p) { timelimit = atoi(myargv[p + 1]); }
    ...
    if (p)
    {
        ...
        else
        {
            ...
            if (p + 2 < myargc) { startmap = myargv[p + 2][0] - '0'; }
            else { startmap = 1; }
        }
        autostart = true;
    }
    ...
    if (p) { startloadgame = atoi(myargv[p + 1]); }
    else
    {
        // Not loading a game
        startloadgame = -1;
