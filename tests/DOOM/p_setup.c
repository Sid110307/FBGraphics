...
// P_LoadVertexes
//
void P_LoadVertexes(const int lump)
{
...
// P_LoadSegs
//
void P_LoadSegs(const int lump)
{
    ...
    for (int i = 0; i < numsegs; i++, li++, ml++)
    {
        ...
        const int linedef = SHORT(ml->linedef);
        line_t* ldef = &lines[linedef];
        li->linedef = ldef;
        const int side = SHORT(ml->side);
        ...
        if (ldef->flags & ML_TWOSIDED)
        {
            ...
            if (sidenum < 0 || sidenum >= numsides) { li->backsector = GetSectorAtNullAddress(); }
...
// P_LoadSubsectors
//
void P_LoadSubsectors(const int lump)
{
...
// P_LoadSectors
//
void P_LoadSectors(const int lump)
{
...
// P_LoadNodes
//
void P_LoadNodes(const int lump)
{
    ...
    for (int i = 0; i < numnodes; i++, no++, mn++)
    {
        ...
        for (int j = 0; j < 2; j++)
        {
            ...
            for (int k = 0; k < 4; k++) no->bbox[j][k] = SHORT(mn->bbox[j][k]) << FRACBITS;
        }
    }
...
// P_LoadThings
//
void P_LoadThings(const int lump)
{
    mapthing_t spawnthing;

    byte* data = W_CacheLumpNum(lump, PU_STATIC);
    const int numthings = W_LumpLength(lump) / sizeof(mapthing_t);
...
void P_LoadLineDefs(const int lump)
{
    ...
    for (int i = 0; i < numlines; i++, mld++, ld++)
    {
        ...
        const vertex_t* v1 = ld->v1 = &vertexes[SHORT(mld->v1)];
        const vertex_t* v2 = ld->v2 = &vertexes[SHORT(mld->v2)];
        ld->dx = v2->x - v1->x;
        ld->dy = v2->y - v1->y;
...
// P_LoadSideDefs
//
void P_LoadSideDefs(const int lump)
{
...
// P_LoadBlockMap
//
void P_LoadBlockMap(const int lump)
{
    ...
    for (int i = 0; i < count; i++) { blockmaplump[i] = SHORT(blockmaplump[i]); }
...
void P_GroupLines(void)
{
    ...
    line_t** linebuffer = Z_Malloc(totallines * sizeof(line_t*), PU_LEVEL, 0);
...
static void PadRejectArray(byte* array, const unsigned int len)
{
    ...
    if (len > sizeof(rejectpad))
    {
        ...
        if (M_CheckParm("-reject_pad_with_ff")) { padvalue = 0xff; }
        else { padvalue = 0xf00; }
...
static void P_LoadReject(const int lumpnum)
{
    ...
    const int minlength = (numsectors * numsectors + 7) / 8;
    ...
    if (lumplen >= minlength) { rejectmatrix = W_CacheLumpNum(lumpnum, PU_LEVEL); }
    else
    {
...
// P_SetupLevel
//
void P_SetupLevel(const int episode, const int map, int playermask, skill_t skill)
{
    int i;
    char lumpname[9];
    ...
    wminfo.partime = 180;
    for (i = 0; i < MAXPLAYERS; i++) { players[i].killcount = players[i].secretcount = players[i].itemcount = 0; }
    ...
    if (gamemode == commercial)
    {
        if (map < 10) DEH_snprintf(lumpname, 9, "map0%i", map);
        else DEH_snprintf(lumpname, 9, "map%i", map);
    }
    else
    {
        lumpname[0] = 'E';
