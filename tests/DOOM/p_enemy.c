...
void P_RecursiveSound(sector_t* sec, const int soundblocks)
{
    sector_t* other;
    ...
    { const line_t* check = sec->lines[i];
        ...
        if (check->flags & ML_SOUNDBLOCK) { if (!soundblocks) P_RecursiveSound(other, 1); }
...
//
void P_NoiseAlert(mobj_t* target, const mobj_t* emmiter)
{
    soundtarget = target;
    validcount++;
...
// P_CheckMeleeRange
//
boolean P_CheckMeleeRange(const mobj_t* actor)
{
    if (!actor->target) return false;

    const mobj_t* pl = actor->target;
    const fixed_t dist = P_AproxDistance(pl->x - actor->x, pl->y - actor->y);
...
boolean P_CheckMissileRange(mobj_t* actor)
{
    ...
    fixed_t dist = P_AproxDistance(actor->x - actor->target->x, actor->y - actor->target->y) - 64 * FRACUNIT;
...
boolean P_Move(mobj_t* actor)
{
    ...
    const boolean try_ok = P_TryMove(actor, tryx, tryy);
...
//
boolean P_LookForPlayers(mobj_t* actor, const boolean allaround)
{
    int c = 0;
    const int stop = actor->lastlook - 1 & 3;
    ...
    for (;; actor->lastlook = actor->lastlook + 1 & 3)
    {
        ...
        const player_t* player = &players[actor->lastlook];
...
void A_Look(mobj_t* actor)
{
    ...
    if (targ && targ->flags & MF_SHOOTABLE)
    {
        ...
        if (actor->flags & MF_AMBUSH) { if (P_CheckSight(actor, actor->target)) goto seeyou; }
        else goto seeyou;
    }
...
void A_Chase(mobj_t* actor)
{
    ...
    {
        actor->angle &= 7 << 29;
        const int delta = actor->angle - (actor->movedir << 29);
    ...
    {
        if (gameskill < sk_nightmare && !fastparm && actor->movecount) { goto nomissile; }
...
void A_PosAttack(mobj_t* actor)
{
    ...
    const int slope = P_AimLineAttack(actor, angle, MISSILERANGE);
...
boolean PIT_VileCheck(mobj_t* thing)
{
    ...
    const int maxdist = thing->info->radius + mobjinfo[MT_VILE].radius;

    if (abs(thing->x - viletryx) > maxdist || abs(thing->y - viletryy) > maxdist) return true; // not actually touching

    corpsehit = thing;
    corpsehit->momx = corpsehit->momy = 0;
    corpsehit->height <<= 2;
    const boolean check = P_CheckPosition(corpsehit, corpsehit->x, corpsehit->y);
    corpsehit->height >>= 2;
        ...
        for (int bx = xl; bx <= xh; bx++)
        {
            for (int by = yl; by <= yh; by++)
            {
                ...
                if (!P_BlockThingsIterator(bx, by, PIT_VileCheck))
                {
                    ...
                    const mobjinfo_t* info = corpsehit->info;
...
void A_Fire(mobj_t* actor)
{
    ...
    const unsigned an = dest->angle >> ANGLETOFINESHIFT;
...
void A_VileAttack(mobj_t* actor)
{
    ...
    const int an = actor->angle >> ANGLETOFINESHIFT;
...
void A_FatAttack1(mobj_t* actor)
{
    ...
    mo->angle += FATSPREAD;
    const int an = mo->angle >> ANGLETOFINESHIFT;
...
void A_SkullAttack(mobj_t* actor)
{
    ...
    A_FaceTarget(actor);
    const angle_t an = actor->angle >> ANGLETOFINESHIFT;
...
void A_PainShootSkull(mobj_t* actor, const angle_t angle)
{
    ...
    {
        if (currentthinker->function.acp1 == (actionf_p1)P_MobjThinker && ((mobj_t*)currentthinker)->type ==
            MT_SKULL)
            count++;
...
static boolean CheckBossEnd(const mobjtype_t motype)
{
    if (gameversion < exe_ultimate)
    {
        if (gamemap != 8) { return false; }

        // Baron death on later episodes is nothing special.

        if (motype == MT_BRUISER && gameepisode != 1) { return false; }
...
void A_BossDeath(const mobj_t* mo)
{
    ...
    else { if (!CheckBossEnd(mo->type)) { return; } }

    // make sure there is a player alive for victory
    for (i = 0; i < MAXPLAYERS; i++) if (playeringame[i] && players[i].health > 0) break;
...
void A_OpenShotgun2(const player_t* player, pspdef_t* psp) { S_StartSound(player->mo, sfx_dbopn); }

void A_LoadShotgun2(const player_t* player, pspdef_t* psp) { S_StartSound(player->mo, sfx_dbload); }
...
{
    const int x = mo->x + (P_Random() - P_Random()) * 2048;
    const int y = mo->y;
    const int z = 128 + P_Random() * 2 * FRACUNIT;
    mobj_t* th = P_SpawnMobj(x, y, z, MT_ROCKET);
    th->momz = P_Random() * 512;
