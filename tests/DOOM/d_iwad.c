...
static const iwad_t iwads[] = {
    ...
    {"tnt.wad", pack_tnt, commercial, "Final Doom: TNT: Evilution"}, {"doom.wad", doom, retail, "Doom"},
    {"doom1.wad", doom, shareware, "Doom Shareware"}, {"chex.wad", pack_chex, shareware, "Chex Quest"},
    {"hacx.wad", pack_hacx, commercial, "Hacx"}, {"freedm.wad", doom2, commercial, "FreeDM"},
    {"freedoom2.wad", doom2, commercial, "Freedoom: Phase 2"}, {"freedoom1.wad", doom, retail, "Freedoom: Phase 1"},
    {"heretic.wad", heretic, retail, "Heretic"}, {"heretic1.wad", heretic, shareware, "Heretic Shareware"},
...
static registry_value_t uninstall_values[] = {
    ...
    {
        HKEY_LOCAL_MACHINE,
        SOFTWARE_KEY "\\Microsoft\\Windows\\CurrentVersion\\" "Uninstall\\Final Doom for Windows 95", "UninstallString",
    },
...
static char* collectors_edition_subdirs[] = {"Doom2", "Final Doom", "Ultimate Doom",};

// Location where Steam is installed

static registry_value_t steam_install_location = {HKEY_LOCAL_MACHINE, SOFTWARE_KEY "\\Valve\\Steam", "InstallPath",};

// Subdirs of the steam install directory where IWADs are found

static char* steam_install_subdirs[] = {
    "steamapps\\common\\doom 2\\base", "steamapps\\common\\final doom\\base", "steamapps\\common\\ultimate doom\\base",
    "steamapps\\common\\heretic shadow of the serpent riders\\base", "steamapps\\common\\hexen\\base",
...
static char* GetRegistryString(const registry_value_t* reg_val)
{
    ...
    if (RegOpenKeyEx(reg_val->root, reg_val->path, 0, KEY_READ, &key) != ERROR_SUCCESS) { return NULL; }
...
static void CheckCollectorsEdition(void)
{
    ...
    {
        char* subpath = M_StringJoin(install_path, DIR_SEPARATOR_S, collectors_edition_subdirs[i], NULL);
...
static void CheckSteamEdition(void)
{
    ...
    {
        char* subpath = M_StringJoin(install_path, DIR_SEPARATOR_S, steam_install_subdirs[i], NULL);
...
static void CheckSteamGUSPatches(void)
{
    ...
    const char* current_path = M_GetStrVariable("gus_patch_path");
    if (current_path != NULL && strlen(current_path) > 0) { return; }

    char* install_path = GetRegistryString(&steam_install_location);

    if (install_path == NULL) { return; }

    const int len = strlen(install_path) + strlen(STEAM_BFG_GUS_PATCHES) + 20;
    char* patch_path = malloc(len);
...
static boolean DirIsFile(const char* path, const char* filename)
{
    ...
    return path_len >= filename_len + 1 && path[path_len - filename_len - 1] == DIR_SEPARATOR && !strcasecmp(
        &path[path_len - filename_len], filename);
}
...
static char* CheckDirectoryHasIWAD(char* dir, char* iwadname)
{
    ...
    if (DirIsFile(dir, iwadname) && M_FileExists(dir)) { return strdup(dir); }

    // Construct the full path to the IWAD if it is located in
    // this directory, and check if it exists.

    if (!strcmp(dir, ".")) { filename = strdup(iwadname); }
    else { filename = M_StringJoin(dir, DIR_SEPARATOR_S, iwadname, NULL); }

    printf("Trying IWAD file:%s\n", filename);

    if (M_FileExists(filename)) { return filename; }
...
static char* SearchDirectoryForIWAD(char* dir, const int mask, GameMission_t* mission)
{
    for (size_t i = 0; i < arrlen(iwads); ++i)
    {
        if ((1 << iwads[i].mission & mask) == 0) { continue; }

        char* filename = CheckDirectoryHasIWAD(dir, DEH_String(iwads[i].name));
...
static GameMission_t IdentifyIWADByName(const char* name, const int mask)
{
    const char* p = strrchr(name, DIR_SEPARATOR);
...
{
#if ORIGCODE
    char* doomwaddir; if (iwad_dirs_built) { return; }
    ...
    doomwaddir = getenv("DOOMWADDIR"); if (doomwaddir != NULL) { AddIWADDir(doomwaddir); }
    ...
    CheckUninstallStrings(); CheckCollectorsEdition(); CheckSteamEdition(); CheckDOSDefaults();
    ...
    AddIWADDir("/usr/share/games/doom"); AddIWADDir("/usr/local/share/games/doom");
...
char* D_FindWADByName(char* name)
{
    ...
    if (M_FileExists(name)) { return name; }
    ...
    for (int i = 0; i < num_iwad_dirs; ++i)
    {
        ...
        if (DirIsFile(iwad_dirs[i], name) && M_FileExists(iwad_dirs[i])) { return strdup(iwad_dirs[i]); }

        // Construct a string for the full path

        char* path = M_StringJoin(iwad_dirs[i], DIR_SEPARATOR_S, name, NULL);

        if (M_FileExists(path)) { return path; }
...
char* D_FindIWAD(const int mask, GameMission_t* mission)
{
    char* result;
    ...
    const int iwadparm = M_CheckParmWithArgs("-iwad", 1);
    ...
    if (iwadparm)
    {
        ...
        if (result == NULL) { I_Error("IWAD file '%s' not found!", iwadfile); }
...
const iwad_t** D_FindAllIWADs(const int mask)
{
    ...
    for (int i = 0; i < arrlen(iwads); ++i)
    {
        ...
        const char* filename = D_FindWADByName(iwads[i].name);
...
char* D_SaveGameIWADName(const GameMission_t gamemission)
{
    ...
    for (size_t i = 0; i < arrlen(iwads); ++i) { if (gamemission == iwads[i].mission) { return iwads[i].name; } }
...
char* D_SuggestIWADName(const GameMission_t mission, const GameMode_t mode)
{
    for (int i = 0; i < arrlen(iwads); ++i)
    {
        if (iwads[i].mission == mission && iwads[i].mode == mode) { return iwads[i].name; }
    }

    return "unknown.wad";
}

char* D_SuggestGameName(const GameMission_t mission, const GameMode_t mode)
{
    for (int i = 0; i < arrlen(iwads); ++i)
    {
