...
static boolean WritePressKey(const byte channel, const byte key, const byte velocity, MEMFILE* midioutput)
{
...
static boolean WriteReleaseKey(const byte channel, const byte key, MEMFILE* midioutput)
{
...
static boolean WritePitchWheel(const byte channel, const short wheel, MEMFILE* midioutput)
{
...
static boolean WriteChangePatch(const byte channel, const byte patch, MEMFILE* midioutput)
{
...
static boolean WriteChangeController_Valued(const byte channel, const byte control, const byte value,
                                            MEMFILE* midioutput)
{
...
static boolean WriteChangeController_Valueless(const byte channel, const byte control, MEMFILE* midioutput)
{
...
static int AllocateMIDIChannel(void)
{
    ...
    for (int i = 0; i < NUM_CHANNELS; ++i) { if (channel_map[i] > max) { max = channel_map[i]; } }
...
static int GetMIDIChannel(const int mus_channel, MEMFILE* midioutput)
{
...
{
    const boolean result = mem_fread(&header->id, sizeof(byte), 4, file) == 4 &&
        ...
        mem_fread(&header->secondarychannels, sizeof(short), 1, file) == 1 && mem_fread(
            &header->instrumentcount, sizeof(short), 1, file) == 1;
...
#define CHECK_MUS_HEADER
    ...
    // Check MUS header
    if (musfileheader.id[0] != 'M' || musfileheader.id[1] != 'U' || musfileheader.id[2] != 'S' || musfileheader.id[3] !=
        0x1A) { return true; }
#endif

    // Seek to where the data is held
    if (mem_fseek(musinput, musfileheader.scorestart, MEM_SEEK_SET) != 0) { return true; }
        ...
        while (!hitscoreend)
        {
            ...
            switch (event)
            {
                ...
                case mus_presskey:
                    ...
                    if (WritePressKey(channel, key, channelvelocities[channel], midioutput)) { return true; }
