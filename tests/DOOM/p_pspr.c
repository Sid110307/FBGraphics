...
// P_SetPsprite
//
void P_SetPsprite(player_t* player, const int position, statenum_t stnum)
{
...
void P_CalcSwing(const player_t* player)
{
    // OPTIMIZE: tablify this.
    // A LUT would allow for different modes,
    //  and add flexibility.

    const fixed_t swing = player->bob;
...
boolean P_CheckAmmo(player_t* player)
{
    ...
    if (player->readyweapon == wp_bfg) count = deh_bfg_cells_per_shot;
    ...
    do
    {
        ...
        else if (player->weaponowned[wp_chaingun] && player->ammo[am_clip]) { player->pendingweapon = wp_chaingun; }
        else if (player->weaponowned[wp_shotgun] && player->ammo[am_shell]) { player->pendingweapon = wp_shotgun; }
        else if (player->ammo[am_clip]) { player->pendingweapon = wp_pistol; }
        else if (player->weaponowned[wp_chainsaw]) { player->pendingweapon = wp_chainsaw; }
        else if (player->weaponowned[wp_missile] && player->ammo[am_misl]) { player->pendingweapon = wp_missile; }
...
void A_WeaponReady(player_t* player, pspdef_t* psp)
{
    ...
    if (player->readyweapon == wp_chainsaw && psp->state == &states[S_SAW]) { S_StartSound(player->mo, sfx_sawidl); }
...
// A_Punch
//
void A_Punch(const player_t* player, pspdef_t* psp)
{
    ...
    const int slope = P_AimLineAttack(player->mo, angle, MELEERANGE);
...
// A_Saw
//
void A_Saw(const player_t* player, pspdef_t* psp)
{
    const int damage = 2 * (P_Random() % 10 + 1);
    angle_t angle = player->mo->angle;
    angle += (P_Random() - P_Random()) << 18;

    // use meleerange + 1 se the puff doesn't skip the flash
    const int slope = P_AimLineAttack(player->mo, angle, MELEERANGE + 1);
    ...
    {
        if ((signed int)(angle - player->mo->angle) < -ANG90 / 20) player->mo->angle = angle + ANG90 / 21;
...
static void DecreaseAmmo(player_t* player, const int ammonum, const int amount)
{
    if (ammonum < NUMAMMO) { player->ammo[ammonum] -= amount; }
    else { player->maxammo[ammonum - NUMAMMO] -= amount; }
}
...
// P_GunShot
//
void P_GunShot(mobj_t* mo, const boolean accurate)
{
...
void A_FireShotgun(player_t* player, pspdef_t* psp)
{
    ...
    for (int i = 0; i < 7; i++) P_GunShot(player->mo, false);
}
...
// A_FireCGun
//
void A_FireCGun(player_t* player, const pspdef_t* psp)
{
    S_StartSound(player->mo, sfx_pistol);
...
//
void A_BFGSpray(const mobj_t* mo)
{
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        int damage = 0;
        for (int j = 0; j < 15; j++) damage += (P_Random() & 7) + 1;
...
// A_BFGsound
//
void A_BFGsound(const player_t* player, pspdef_t* psp) { S_StartSound(player->mo, sfx_bfg); }
...
{
    // remove all psprites
    for (int i = 0; i < NUMPSPRITES; i++) player->psprites[i].state = NULL;
