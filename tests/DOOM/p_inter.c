...
boolean P_GiveAmmo(player_t* player, const ammotype_t ammo, int num)
{
    ...
    const int oldammo = player->ammo[ammo];
...
//
boolean P_GiveWeapon(player_t* player, const weapontype_t weapon, const boolean dropped)
{
    boolean gaveammo;
    boolean gaveweapon;
...
//
boolean P_GiveBody(player_t* player, const int num)
{
...
//
boolean P_GiveArmor(player_t* player, const int armortype)
{
...
// P_GiveCard
//
void P_GiveCard(player_t* player, const card_t card)
{
...
// P_GivePower
//
boolean P_GivePower(player_t* player, const int /*powertype_t*/ power)
{
...
void P_DamageMobj(mobj_t* target, const mobj_t* inflictor, mobj_t* source, int damage)
{
    ...
    if (inflictor && !(target->flags & MF_NOCLIP) && (!source || !source->player || source->player->readyweapon !=
        wp_chainsaw))
    {
    ...
    if (player)
    {
        ...
        if (damage < 1000 && (player->cheats & CF_GODMODE || player->powers[pw_invulnerability])) { return; }
        ...
        const int temp = damage < 100 ? damage : 100;
